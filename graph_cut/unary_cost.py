...
def compute_histograms(image, labels, K, bins=16):
    ...
    for k in range(K):
        mask = labels == k
        pixels = image[mask]
        hist = np.histogramdd(pixels, bins=bins, range=[(0, 256), (0, 256), (0, 256)])[
            0
        ]
...
def compute_unary_term(image, labels, histograms, K, bins=16):
    ...
    for k in range(K):
        ...
        for i in range(h):
            for j in range(w):
                pixel = image[i, j]
                bin_idx = np.floor(pixel / bins).astype(
                    int
                )  # Map pixel to histogram bin
                unary[i, j, k] = -np.log(
                    hist[bin_idx[0], bin_idx[1], bin_idx[2]] + 1e-8
                )  # Avoid log(0)
...
def unary_term_L2_sCIELAB(image, labels_colors):
    ...
    labels_colors_lab = cv2.cvtColor(
        np.uint8(labels_colors[None, :, :]), cv2.COLOR_RGB2LAB
    )[0].astype(np.float32)

    # Compute L2 norm
    unary_term = np.linalg.norm(
        image_lab[:, :, None, :] - labels_colors_lab[None, None, :, :], axis=-1
    )

...
class Alpha_and_recycle(Runner):
    def __init__(self, image, unary, pairwise, K, max_iterations):
        ...
        self.Alpha_exp=Alpha_expansion2(
            image=image, unary=unary, pairwise=pairwise, K=K, max_iterations=max_iterations)

        self.recycle=Recycle(
            image=image, unary=unary, pairwise=pairwise, K=K)
    ...
    def run(self, image):
        ...
        labels = np.ones((self.h, self.w), dtype=np.int32) * self.epsilon #! Initially no labels are assigned
        ...
        labels, assigned_labels, self.cst, unary, pairwise= self.recycle.run(image=image)
        print("after recycling", show_segmentation(image, labels, K,title="after recycling"))
        ...
        print("after init unassigned labels",show_segmentation(image, labels, K,title="after init unassigned labels"))
        
        # Run the Alpha expansion algorihtm
        self.Alpha_exp=Alpha_expansion2(
            image=image, unary=unary, pairwise=pairwise, K=K, max_iterations=max_iterations)
        # now run the Alpha expansion
        labels=self.Alpha_exp.run(image=image,assigned_labels=assigned_labels,init_labels=labels)

...
class Alpha_expansion2(Runner):
    def __init__(self, image, unary, pairwise, K, max_iterations):
        """
        Alpha Expansion without the recycling step
    ...
    def construct_graph(self, alpha, labels):
        ...
        for i in range(h):
            for j in range(w):
                ...
                pixel_index = i * w + j
                if (
                    self.assigned_labels[i, j] != self.epsilon
                ):  # if the pixel is already assigned
                    continue
                if labels[i, j] == alpha:
                    graph.add_tedge(
                ...
                if i < h - 1:
                    ...
                    if (
                        self.assigned_labels[i, j] == self.epsilon
                        and self.assigned_labels[i + 1, j] == self.epsilon
                    ):  # if the pixel is already assigned
                        ...
                        if labels[i, j] != labels[i + 1, j]:
                            ...
                            graph.add_tedge(
                                aux_node,
                                0,
                                pairwise[i, j, labels[i, j], labels[i + 1, j]],
                            )
                ...
                if j < w - 1:
                    if (
                        self.assigned_labels[i, j] == self.epsilon
                        and self.assigned_labels[i, j + 1] == self.epsilon
                    ):  # if the pixel is already assigned
                        ...
                        if labels[i, j] != labels[i, j + 1]:
                            ...
                            graph.add_tedge(
                                aux_node,
                                0,
                                pairwise[i, j, labels[i, j], labels[i, j + 1]],
                            )
                        else:
                            graph.add_edge(
                                nodes[pixel_index],
            ...
            for alpha in range(K):
                ...
                if nv_energy < energy:
                    ...
                    show_segmentation(
                        ...
                        labels,
                        K,
                        title=f"Alpha_exp: iteration {_} alpha {alpha}",
                    )
                    print("energy", self.compute_energy(labels, unary, pairwise))
                if self.node_changed == False:
                    print(
                        "no node have changed during the whole iteration, we stop here"
                    )
                    break
            # if _ % 5 == 0:
